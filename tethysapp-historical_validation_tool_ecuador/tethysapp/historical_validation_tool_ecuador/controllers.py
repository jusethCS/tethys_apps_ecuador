...
def get_daily_average_plot(merged_sim, merged_cor, code, name):
    ...
    # PLot Layout
    layout = go.Layout(
        title='Daily Average Streamflow for <br> {0} - {1}'.format(code.upper(), name),
...
def get_monthly_average_plot(merged_sim, merged_cor, code, name):
    ...
    # PLot Layout
    layout = go.Layout(
        title='Monthly Average Streamflow for <br> {0} - {1}'.format(code.upper(), name),
...
def get_acumulated_volume_plot(merged_sim, merged_cor, code, name):
    ...
    # Plot layouts
    layout = go.Layout(
                title='Observed & Simulated Volume at<br> {0} - {1}'.format(code.upper(), name),
...
@controller(name='get_stations',url='historical-validation-tool-ecuador/get-stations')
def get_stations(request):
    ...
    stations = to_geojson(
        ...
        properties = ["cuenca", "codigo", "nombre", "latitud", "longitud", "elevacion", "comid", "rio", 
                      "provincia", "parroquia", "canton", "alerta"]
    )
...
def get_historical_data(request):
    ...
    global corrected_forecast_table
...
@controller(name='get_metrics',url='historical-validation-tool-ecuador/get-metrics')
...
@controller(name='get_forecast_table',url='historical-validation-tool-ecuador/get-forecast-table')
def get_forecast_table(request):
    return HttpResponse(forecast_table)
